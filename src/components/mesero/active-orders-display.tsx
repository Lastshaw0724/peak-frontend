...
import { WaiterOrderCard } from '../mesero/waiter-order-card';
    ...
    <div className="p-4 sm:p-6 lg:p-8 h-full">
      <div className="mb-6">
        <div className="relative">
            ...
            <Input
                placeholder="Search by ID, customer, or waiter..."
                value={searchTerm}
      ...
      {activeOrders.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-[calc(100vh-12rem)] text-center text-muted-foreground">
          ...
          <h2 className="text-3xl font-headline">No Active Orders</h2>
          <p className="mt-2 text-lg">Confirmed orders will appear here.</p>
        </div>
      ) : filteredOrders.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-[calc(100vh-12rem)] text-center text-muted-foreground">
          <Search className="w-24 h-24 mb-4 text-primary/50" />
          <h2 className="text-3xl font-headline">No orders found</h2>
          <p className="mt-2 text-lg">Try a different search term.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <section>
            <h2 className="text-2xl font-headline text-red-400 mb-4 pb-2 border-b-2 border-red-400/50">New ({newOrders.length})</h2>
            <div className="space-y-4">
              {newOrders.map(order => <WaiterOrderCard key={order.id} order={order} />)}
            </div>
          </section>
          <section>
            <h2 className="text-2xl font-headline text-primary mb-4 pb-2 border-b-2 border-primary/50">Preparing ({preparingOrders.length})</h2>
            <div className="space-y-4">
              {preparingOrders.map(order => <WaiterOrderCard key={order.id} order={order} />)}
            </div>
          </section>
          <section>
            <h2 className="text-2xl font-headline text-green-400 mb-4 pb-2 border-b-2 border-green-400/50">Ready ({readyOrders.length})</h2>
            <div className="space-y-4">
