...
import { usarDescuentos } from '@/hooks/usar-descuentos';
import { usarAutenticacion } from '@/hooks/usar-autenticacion';
import type { Descuento, OpcionFactura } from '@/lib/tipos';
...
export function ResumenPedido() {
  const { currentOrder, updateItemQuantity, removeItemFromOrder, submitOrder, clearCurrentOrder, currentOrderDetails } = usarPedidos();
  const { activeTable, setActiveTable, updateTableStatus } = usarMesas();
  const { discounts } = usarDescuentos();
  ...
  const [invoiceOption, setInvoiceOption] = useState<OpcionFactura>('none');
            ...
            <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="grid gap-6 py-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <RadioGroup defaultValue="none" value={invoiceOption} onValueChange={(v) => setInvoiceOption(v as OpcionFactura)} className="space-y-1 pt-1">
                             <div className="flex items-center space-x-2">
