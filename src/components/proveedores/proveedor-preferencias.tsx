...
interface PreferencesContextType extends Preferencias, SetterFunctions {
  isLoading: boolean;
}
...
export const ProveedorPreferencias = ({ children }: { children: ReactNode }) => {
  const [preferences, setPreferences] = useState<Preferencias>(defaultPreferences);
  const [isLoading, setIsLoading] = useState(true);
  const { toast } = useToast();

  const applyTheme = useCallback((prefs: Preferencias) => {
  ...
  const savePreferences = (newPrefs: Preferencias) => {
