...
export function GestorRolesUsuario() {
    const { user: currentUser, users, updateUserRole, deleteUser, addUser, updateUserPassword } = usarAutenticacion();
    const { toast } = useToast();
    const [userRoles, setUserRoles] = useState<Record<string, RolUsuario>>({});
    ...
    const handleRoleChange = (userId: string, role: RolUsuario) => {
                        ...
                        <TableBody>
                             {manageableUsers.map(user => (
                                <TableRow key={user.id}>
                                    ...
                                    <TableCell>
                                        <Select value={userRoles[user.id] || user.role} onValueChange={(value: RolUsuario) => handleRoleChange(user.id, value)}>
